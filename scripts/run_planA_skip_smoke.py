#!/usr/bin/env python3
"""
Plan A + full skip smoke test.

Compares:
  1. Baseline (dense, CFG=4.0)
  2. Plan A only: sparse from 6, bg refresh every 5 (1.65x)
  3. Plan A + full_skip=3: sparse + skip every 3rd step entirely
  4. Plan A + full_skip=2: sparse + skip every 2nd step entirely
  5. Plan A (bg ref/3) + full_skip=3
...
def main():
    ...
    experiments = [
        {
            "name": "planA_skip-1-baseline",
            "desc": "Baseline (dense, CFG=4.0)",
            "method": "baseline",
        },
        {
            "name": "planA_skip-2-planA-only",
            "desc": "Plan A: from 6, bg ref/5 (no full skip)",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=5),
        },
        {
            "name": "planA_skip-3-fskip3",
            "desc": "Plan A ref/5 + full_skip=3",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=5, full_skip_interval=3),
        },
        {
            "name": "planA_skip-4-fskip2",
            "desc": "Plan A ref/5 + full_skip=2",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=5, full_skip_interval=2),
        },
        {
            "name": "planA_skip-5-ref3-fskip3",
            "desc": "Plan A ref/3 + full_skip=3",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=3, full_skip_interval=3),
        },
    ]

    print(f"=== Plan A + Full Skip Smoke Test ({len(experiments)} configs) ===")
    print(f"Prompts: {len(test_prompts)}, Seed: {seed}, Steps: {steps}")
    print(f"Key: full_skip_interval skips entire transformer on some steps")
    print()
    ...
    ref_time = all_results["planA_skip-1-baseline"]["mean_time"]
    ref_clip = all_results["planA_skip-1-baseline"]["mean_clip"]
    ...
    print("=" * 110)
    print()
    print("=== Compare images in outputs/planA_skip-*/images/ ===")
