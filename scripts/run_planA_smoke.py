#!/usr/bin/env python3
"""
Plan A smoke test: fg always computed, bg cached/extrapolated.

Compares:
  1. Baseline (dense, CFG=4.0)
  2. Old approach: sparse from step 20, skip entire steps (skip_step_interval=2)
  3. Plan A: sparse from step 6, bg refresh every 3 steps
  4. Plan A: sparse from step 6, bg refresh every 5 steps
  5. Plan A: sparse from step 10, bg refresh every 3 steps
...
def main():
    ...
    experiments = [
        {
            "name": "planA-1-baseline",
            "desc": "Baseline (dense, CFG=4.0)",
            "method": "baseline",
        },
        {
            "name": "planA-2-old-from20",
            "desc": "Old: sparse from 20, skip=2 (old behavior disabled)",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=20, skip_step_interval=2),
        },
        {
            "name": "planA-3-from6-ref3",
            "desc": "Plan A: sparse from 6, bg refresh every 3",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=3),
        },
        {
            "name": "planA-4-from6-ref5",
            "desc": "Plan A: sparse from 6, bg refresh every 5",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=5),
        },
        {
            "name": "planA-5-from10-ref3",
            "desc": "Plan A: sparse from 10, bg refresh every 3",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=10, skip_step_interval=3),
        },
    ]

    print(f"=== Plan A Smoke Test ({len(experiments)} configs) ===")
    print(f"Prompts: {len(test_prompts)}, Seed: {seed}, Steps: {steps}")
    print(f"Key change: fg ALWAYS computed, bg uses output-level extrapolation")
    print()
    ...
    print()
    print("=" * 100)
    print(f"{'Config':<50} {'Time':>6} {'Speed':>6} {'CLIP':>8} {'dCLIP':>7} {'Edge':>8}")
    print("=" * 100)

    ref_time = all_results["planA-1-baseline"]["mean_time"]
    ref_clip = all_results["planA-1-baseline"]["mean_clip"]
    ...
    for name, r in all_results.items():
        ...
        dc = mc - ref_clip
        edge = r["mean_edge"]
        print(f"  {r['desc']:<48} {t:5.2f}s {speedup:5.2f}x {mc:7.4f} {dc:+6.4f} {edge:7.4f}")

    print("=" * 100)
    print()
    print("=== Compare images in outputs/planA-*/images/ ===")
