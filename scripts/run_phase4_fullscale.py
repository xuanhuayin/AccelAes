#!/usr/bin/env python3
"""
Phase 4 Full Scale — top configs + baseline.
50 prompts × 3 seeds = 150 images per config.
"""
...
def main():
    ...
    prompts = load_prompts("prompts/prompts_dev.txt")[:50]
    seeds = [0, 1, 2]
    ...
    experiments = [
        {
            "name": "fullscale_p3-baseline-ref",
            "desc": "P3-baseline-ref (grid + linear)",
            "method": "v1",
            "kwargs": dict(
    ...
    print(f"=== Phase 4 Full Scale ({len(experiments)} configs × {total_images} imgs) ===")
    print(f"Prompts: {len(prompts)}, Seeds: {seeds}, Steps: {steps}, CFG: {cfg_scale}")
    print()
    ...
    for exp_idx, exp in enumerate(experiments):
        ...
        for pi, prompt in enumerate(prompts):
            for seed in seeds:
                ...
                if count % 30 == 0 or count == total_images:
                    pct = count / total_images * 100
                    print(f"    [{count}/{total_images}] ({pct:.0f}%) {elapsed:.1f}s")
    ...
    print("=== Full Scale complete! ===")
    exp_names = " ".join(f"outputs/{e['name']}" for e in experiments)
    print(f"Run eval:\n  python scripts/run_eval.py --exp_dir <dir>")
