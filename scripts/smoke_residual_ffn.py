"""
Smoke test: compare baseline vs sparse_residual_ffn + fskip2 on 3 prompts.
Prints per-image CLIP score and per-run wall time.
"""
import sys, time, os
...
def clip_score(image, text):
    ...
    return out.logits_per_image.item() / 100.0  # cosine-like score

configs = [
    dict(name="baseline",        fn="generate",             kwargs=dict(steps=28, cfg_scale=7.0)),
    dict(name="residual_fskip2", fn="generate_accelerated", kwargs=dict(
        steps=28, mask_type="cfg_magnitude", skip_ratio=0.5,
        full_skip_interval=2, sparse_residual_ffn=True, n_segments=64,
    )),
]
...
for cfg in configs:
    ...
    for prompt in PROMPTS:
        for seed in SEEDS:
            ...
            scores.append(s)
            print(f"  [{cfg['name']}] seed={seed} clip={s:.4f}  prompt={prompt[:40]}")
