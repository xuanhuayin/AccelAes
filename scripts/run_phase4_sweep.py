#!/usr/bin/env python3
"""
Phase 4 Quick Sweep — single-process, loads model once.
10 prompts × 3 seeds = 30 images per config, 7 configs.
"""
...
def main():
    ...
    print(f"=== Phase 4 Quick Sweep ({len(experiments)} configs × {total_images} imgs) ===")
        ...
        for pi, prompt in enumerate(prompts):
            for seed in seeds:
                ...
                if count % 10 == 0 or count == total_images:
                    print(f"    [{count}/{total_images}] {elapsed:.1f}s")
    ...
    print(f"  python scripts/run_eval.py --exp_dirs {exp_names}")
