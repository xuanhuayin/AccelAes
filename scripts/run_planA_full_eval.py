#!/usr/bin/env python3
"""
Plan A full evaluation: generate 60 images + run 8 metrics.

Configs:
  1. Baseline (dense, CFG=4.0)  — reuse existing if available
  2. Plan A: sparse_start=6, skip_interval=5 (best from smoke test)
  3. Plan A: sparse_start=6, skip_interval=3 (more aggressive bg cache)
...
def generate_images(wrapper, prompts, seeds, resolution, steps, exp_config, output_base):
    """Generate images for one experiment config. Returns (img_dir, mean_time)."""
    name = exp_config["name"]
    exp_dir = setup_output_dir(output_base, name)
    img_dir = os.path.join(exp_dir, "images")

    n_images = len(prompts) * len(seeds)
    print(f"\n=== Generating {n_images} images: {exp_config['desc']} ===")
    ...
    print(f"  Mean: {mean_time:.2f}s/img, Total: {total_time:.1f}s")
...
def main():
    ...
    experiments = [
        {
            "name": "planA_eval-1-baseline",
            "desc": "Baseline (dense, CFG=4.0)",
            "method": "baseline",
        },
        {
            "name": "planA_eval-2-from6-ref5",
            "desc": "Plan A: from 6, bg refresh every 5",
            "method": "accel_dual",
            "kwargs": dict(**common, sparse_start_step=-1, skip_step_interval=5),
        },
        {
            "name": "planA_eval-3-from6-ref3",
            "desc": "Plan A: from 6, bg refresh every 3",
            "method": "accel_dual",
    ...
    for exp in experiments:
        _, mean_time = generate_images(
            wrapper, prompts, seeds, resolution, steps, exp, output_base
        )
        measured_times[exp["name"]] = mean_time

    # Free GPU memory for metrics
    del wrapper
    gc.collect()
    torch.cuda.empty_cache()
    ...
    for exp in experiments:
        ...
        line = f"  {desc:<43} {t:5.2f}s {speedup:5.2f}x"
    ...
    summary_path = os.path.join(output_base, "planA_full_eval_summary.json")
